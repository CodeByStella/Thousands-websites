...
def fetch_site_list(
    session: requests.Session, post_num_now: int, post_num_add: int
) -> Optional[str]:
    """Fetch site list from API endpoint."""
    try:
        response = session.post(
            API_ENDPOINT,
            data={"post_num_now": post_num_now, "post_num_add": post_num_add},
            timeout=TIMEOUT,
        )
    ...
    for li in soup.find_all("li", class_="c-post-list__item"):
        try:
            ...
            if credit_detail:
                ...
                copyright_elem = credit_detail.find(
                    "dt", string=re.compile("著作権所有")
                )
                if copyright_elem:
                    ...
                    if copyright_dd:
                        ...
                        if copyright_span:
                            credit_info["copyright"] = copyright_span.get_text(
                                strip=True
                            )
                ...
                creator_source_elem = credit_detail.find(
                    "dt", string=re.compile("制作者情報引用元")
                )
                if creator_source_elem:
                    ...
                    if creator_source_dd:
                        ...
                        if creator_source_span:
                            credit_info["creator_source"] = (
                                creator_source_span.get_text(strip=True)
                            )
...
def process_detail_page(
    session: requests.Session, detail_url: str, project_dir: Path
) -> bool:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            try:
                ...
                logger.info(
                    f"[HTTP] Detail Headers: Content-Type={response.headers.get('Content-Type')}, Content-Encoding={response.headers.get('Content-Encoding')}, Transfer-Encoding={response.headers.get('Transfer-Encoding')}"
                )
                logger.info(
                    f"[HTTP] Detail raw content length: {len(response.content)} bytes"
                )
            except requests.exceptions.ConnectionError as e:
                logger.error(
                    f"Connection error fetching detail page {detail_url} (attempt {attempt + 1}/{MAX_RETRIES}): {e}"
                )
                ...
                logger.error(
                    f"Timeout fetching detail page {detail_url} (attempt {attempt + 1}/{MAX_RETRIES}): {e}"
                )
                if attempt < MAX_RETRIES - 1:
                ...
                logger.error(
                    f"Request error fetching detail page {detail_url} (attempt {attempt + 1}/{MAX_RETRIES}): {e}"
                )
                if attempt < MAX_RETRIES - 1:
                    time.sleep(REQUEST_INTERVAL * (attempt + 1))
                    continue
            ...
            logger.info(
                f"[HTTP] Detail decoded-bytes length (after Content-Encoding handling): {len(content_bytes)} bytes"
            )
            ...
            if not encoding or encoding.lower() == "iso-8859-1":
                try:
                    ...
                    for test_encoding in [
                        ...
                        try:
                            test_decode = content_bytes.decode(
                                test_encoding, errors="strict"
                            )
                            if (
                                "<html" in test_decode.lower()
                                or "<!doctype" in test_decode.lower()
                            ):
            ...
            if not html_content or len(html_content.strip()) < 100:
                logger.error(
                    f"Detail page HTML content is too short or empty for {detail_url}"
                )
                return False
            ...
            if not (
                ...
                if html_start > 0:
                    ...
                    logger.error(
                        f"Could not find HTML start markers in detail page for {detail_url}"
                    )
                    return False
            ...
            try:
                ...
                if (
                    not soup.find("html")
                    and not soup.find("body")
                    and len(soup.find_all()) == 0
                ):
                    logger.error(
                        f"BeautifulSoup failed to parse detail page HTML for {detail_url}"
                    )
                    return False
            except Exception as e:
                logger.error(
                    f"BeautifulSoup parsing error for detail page {detail_url}: {e}"
                )
                return False
            ...
            if inline_css.strip():
                ...
                with open(
                    inline_css_path, "w", encoding="utf-8", errors="replace"
                ) as f:
                    f.write(inline_css)
            ...
            for img in soup.find_all("img", src=True):
                ...
                if not (
                    parsed_img_url.netloc == urlparse(base_url).netloc
                    or not parsed_img_url.netloc
                ):
                    continue
                ...
                if not any(
                    img_filename.lower().endswith(ext)
                    for ext in [
                        ".jpg",
                        ".jpeg",
                        ".png",
                ...
                if img.get("srcset"):
                    ...
                    for part in srcset.split(","):
                        ...
                        if (
                            parsed_srcset_url.netloc == urlparse(base_url).netloc
                            or not parsed_srcset_url.netloc
                        ):
                            srcset_filename = (
                                os.path.basename(parsed_srcset_url.path)
                                or f"image_info_{img_count}.jpg"
                            )
                            if "?" in srcset_filename:
                                srcset_filename = srcset_filename.split("?")[0]
                            if not any(
                                srcset_filename.lower().endswith(ext)
                                for ext in [
                                    ".jpg",
                                    ".jpeg",
                                    ".png",
                            ...
                            if download_image(session, srcset_abs_url, srcset_path):
                                srcset_parts.append(
                                    f"assets/images/{srcset_filename} {descriptor}"
                                )
                                img_count += 1
                            else:
                    ...
                    if url_match:
                        ...
                        if bg_url.startswith("http") or bg_url.startswith("//"):
                            ...
                            if parsed_bg_url.netloc == urlparse(base_url).netloc:
                                bg_filename = (
                                    os.path.basename(parsed_bg_url.path)
                                    or f"bg_info_{img_count}.jpg"
                                )
                                if "?" in bg_filename:
                                    bg_filename = bg_filename.split("?")[0]
                                if not any(
                                    bg_filename.lower().endswith(ext)
                                    for ext in [
                                        ".jpg",
                                        ".jpeg",
                                        ".png",
                                ...
                                if download_image(session, bg_img_url, bg_path):
                                    new_style = style_attr.replace(
                                        bg_url, f"assets/images/{bg_filename}"
                                    )
            ...
            with open(
                info_path, "w", encoding="utf-8", errors="replace", newline=""
            ) as f:
                f.write(updated_html)

            logger.info(
                f"Downloaded detail page with {css_count} CSS files, {js_count} JS files, and {img_count} images"
            )
            return True

        except requests.RequestException as e:
            logger.warning(
                f"Error fetching detail page (attempt {attempt + 1}/{MAX_RETRIES}): {e}"
            )
            if attempt < MAX_RETRIES - 1:
                time.sleep(REQUEST_INTERVAL)
            else:
                logger.error(
                    f"Failed to fetch detail page after {MAX_RETRIES} attempts: {detail_url}"
                )
                return False
        except Exception as e:
            logger.error(
                f"Error processing detail page for {detail_url}: {e}", exc_info=True
            )
            return False
...
def download_file(
    session: requests.Session, url: str, save_path: Path, is_text: bool = False
) -> bool:
    ...
    try:
        ...
        if is_text:
            ...
            logger.debug(
                f"[HTTP] Headers: Content-Type={response.headers.get('Content-Type')}, Content-Encoding={response.headers.get('Content-Encoding')}"
            )
            content_bytes = _get_decoded_bytes(response)
            ...
            try:
                ...
                logger.debug(
                    f"Encoding error with {encoding} for {url}, using UTF-8: {e}"
                )
            ...
            with open(
                save_path, "w", encoding="utf-8", errors="replace", newline=""
            ) as f:
                f.write(content)
        else:
...
def download_image(session: requests.Session, url: str, save_path: Path) -> bool:
    ...
    try:
        ...
        if content_type and not content_type.startswith("image/"):
            logger.debug(
                f"Skipping non-image content: {url} (content-type: {content_type})"
            )
            return False
...
def fetch_website_content(
    session: requests.Session, site_url: str, project_dir: Path
) -> Dict[str, bool]:
    """Fetch website HTML, CSS, and JavaScript files and update HTML paths."""
    results = {"html": False, "css": False, "js": False}
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"[HTTP] Headers: Content-Type={response.headers.get('Content-Type')}, Content-Encoding={response.headers.get('Content-Encoding')}, Transfer-Encoding={response.headers.get('Transfer-Encoding')}, Vary={response.headers.get('Vary')}"
            )
        ...
        content_bytes = _get_decoded_bytes(response)
        logger.info(
            f"[HTTP] Decoded-bytes length (after Content-Encoding handling): {len(content_bytes)} bytes"
        )
        ...
        try:
            ...
            logger.info(
                f"[ENCODING] UTF-8 decode successful, content length: {len(test_content)} chars"
            )
            ...
            try:
                ...
                if len(all_tags) > 0:
                    logger.info(
                        f"[PARSING] First 10 tag names: {[tag.name for tag in all_tags[:10]]}"
                    )
                    html_content = test_content
                    encoding = "utf-8"
                    logger.info(
                        f"[SUCCESS] Successfully decoded with UTF-8 (found {len(all_tags)} tags)"
                    )
                else:
                    tried_encodings.append("utf-8 (BeautifulSoup found 0 tags)")
                    logger.warning(
                        f"[PARSING] BeautifulSoup found 0 tags - validation failed"
                    )
            except Exception as e:
                # BeautifulSoup failed, but try the content anyway
                logger.warning(
                    f"[PARSING] BeautifulSoup parsing exception: {type(e).__name__}: {e}"
                )
        ...
        if not html_content:
            ...
            for test_encoding in encodings_to_try:
                ...
                try:
                    ...
                    logger.info(
                        f"[ENCODING] {test_encoding} decode successful, content length: {len(test_content)} chars"
                    )

                    # Use BeautifulSoup to validate
                    try:
                        logger.info(
                            f"[PARSING] Attempting BeautifulSoup parse with {test_encoding}..."
                        )
                        test_soup = BeautifulSoup(test_content, "html.parser")
                        all_tags = test_soup.find_all()
                        logger.info(
                            f"[PARSING] BeautifulSoup found {len(all_tags)} tags"
                        )
                        ...
                        if len(all_tags) > 0:
                            ...
                            encoding = test_encoding
                            logger.info(
                                f"[SUCCESS] Successfully decoded with {test_encoding} (found {len(all_tags)} tags)"
                            )
                            break
                        else:
                            tried_encodings.append(
                                f"{test_encoding} (BeautifulSoup found 0 tags)"
                            )
                            logger.warning(
                                f"[PARSING] {test_encoding} found 0 tags - validation failed"
                            )
                    except Exception as e:
                        # BeautifulSoup failed, but try the content anyway
                        logger.warning(
                            f"[PARSING] BeautifulSoup parsing exception for {test_encoding}: {type(e).__name__}: {e}"
                        )
                        logger.warning(
                            f"[PARSING] Using content anyway despite parsing error"
                        )
                        html_content = test_content
                        encoding = test_encoding
                        break
                    ...
                    logger.warning(
                        f"[ENCODING] {test_encoding} decode failed: {type(e).__name__}: {e}"
                    )

        # Final fallback: use UTF-8 anyway
        if not html_content:
            logger.error(
                f"[ENCODING] All encoding attempts failed validation for {site_url}"
            )
            ...
            encoding = "utf-8"
            logger.info(
                f"[ENCODING] Final fallback content length: {len(html_content)} chars"
            )

        logger.info(f"[VALIDATION] Final encoding selected: {encoding}")
        logger.info(
            f"[VALIDATION] HTML content length before normalization: {len(html_content)} chars"
        )
        ...
        if html_content:
            ...
            logger.info(
                f"[VALIDATION] HTML content length after normalization: {len(html_content)} chars"
            )

        # Validate HTML content length (basic check)
        if not html_content or len(html_content) < 100:
            logger.error(
                f"[VALIDATION] HTML content is too short or empty for {site_url} (length: {len(html_content) if html_content else 0})"
            )
            return results

        logger.info(
            f"[VALIDATION] HTML content length check passed: {len(html_content)} chars"
        )

        # Parse HTML to find CSS and JS files BEFORE downloading
        # Use html.parser which handles encoding better than lxml
        logger.info(
            f"[PARSING] Starting final BeautifulSoup parse for asset extraction..."
        )
        try:
            soup = BeautifulSoup(html_content, "html.parser")
            ...
            if len(all_tags) > 0:
                logger.info(
                    f"[PARSING]   - First 15 tag names: {[tag.name for tag in all_tags[:15]]}"
                )

            # More lenient validation - accept if we have any tags, even without html/body
            if len(all_tags) == 0:
                logger.error(
                    f"[PARSING] BeautifulSoup failed to parse HTML for {site_url} - no elements found at all"
                )
                logger.error(
                    f"[PARSING] Content preview: {html_content[:500] if html_content else 'None'}"
                )
                return results
            elif not html_tag and not body_tag:
                # If we have tags but no html/body, log warning but continue (might be a fragment)
                logger.warning(
                    f"[PARSING] No <html> or <body> tags found for {site_url}, but found {len(all_tags)} other tags. Continuing anyway."
                )
                logger.info(
                    f"[PARSING] First few tags: {[tag.name for tag in all_tags[:10]]}"
                )

            logger.info(
                f"[PARSING] Validation passed, proceeding with asset extraction..."
            )
        except Exception as e:
            logger.error(
                f"[PARSING] BeautifulSoup parsing exception: {type(e).__name__}: {e}"
        ...
        logger.info(
            f"[DOMAIN] Base domain: {base_domain}, normalized: {base_domain_no_www}"
        )
        ...
        for link in soup.find_all("link", rel="stylesheet"):
            ...
            counter = 1
            while (
                assets_dir / css_filename
            ).exists() and css_url not in downloaded_css:
                name, ext = os.path.splitext(original_filename)
                css_filename = f"{name}_{counter}{ext}"
                counter += 1
        ...
        if css_downloads:
            with ThreadPoolExecutor(max_workers=min(5, len(css_downloads))) as executor:
                css_futures = {
                    executor.submit(download_file, session, url, path, True): (
                        url,
                        filename,
                        link_elem,
                    )
        ...
        for img in soup.find_all("img", src=True):
            ...
            is_same_domain = (
                not parsed_img_url.netloc  # Relative path
                or parsed_img_url.netloc.lower() == base_domain  # Exact match
                or img_domain_no_www
                == base_domain_no_www  # Same domain (www vs non-www)
            )

            if not is_same_domain:
                logger.debug(
                    f"[IMAGE] Skipping external image: {img_url} (domain: {img_domain}, base: {base_domain})"
                )
                continue
            ...
            if not any(
                ...
                ext_match = re.search(
                    r"\.(jpg|jpeg|png|gif|webp|svg|ico)", parsed_img_url.path.lower()
                )
                if ext_match:
                    img_filename += ext_match.group(0)
                else:
            ...
            counter = 1
            while (
                images_dir / img_filename
            ).exists() and img_url not in downloaded_images:
                name, ext = os.path.splitext(original_filename)
                img_filename = f"{name}_{counter}{ext}"
                counter += 1
        ...
        if img_downloads:
            with ThreadPoolExecutor(
                max_workers=min(10, len(img_downloads))
            ) as executor:
                img_futures = {
        ...
        for img in soup.find_all("img", src=True):
            ...
            if img.get("srcset"):
                ...
                for part in srcset.split(","):
                    ...
                    parsed_srcset_url = urlparse(srcset_abs_url)
                    srcset_domain = (
                        parsed_srcset_url.netloc.lower()
                        if parsed_srcset_url.netloc
                        else ""
                    )
                    ...
                    is_same_domain = (
                        not parsed_srcset_url.netloc  # Relative path
                        or parsed_srcset_url.netloc.lower()
                        == base_domain  # Exact match
                        or srcset_domain_no_www
                        == base_domain_no_www  # Same domain (www vs non-www)
                    )
                    ...
                    if is_same_domain:
                        ...
                        if srcset_abs_url in downloaded_images:
                            ...
                            srcset_parts.append(
                                f"assets/images/{downloaded_images[srcset_abs_url]} {descriptor}"
                            )
                        else:
                            # Try to download
                            srcset_filename = (
                                os.path.basename(parsed_srcset_url.path)
                                or f"image_{img_count}.jpg"
                            )
                            if "?" in srcset_filename:
                                srcset_filename = srcset_filename.split("?")[0]
                            if not any(
            ...
            if "background-image" in style_attr or "background:" in style_attr:
                ...
                if url_match:
                    ...
                    if bg_url.startswith("http") or bg_url.startswith("//"):
                        ...
                        parsed_bg_url = urlparse(bg_img_url)
                        bg_domain = (
                            parsed_bg_url.netloc.lower() if parsed_bg_url.netloc else ""
                        )
                        ...
                        # Only download from same domain (allow www vs non-www)
                        is_same_domain = (
                            parsed_bg_url.netloc.lower() == base_domain  # Exact match
                            or bg_domain_no_www
                            == base_domain_no_www  # Same domain (www vs non-www)
                        )
        ...
        logger.info(
            f"Downloaded {css_count} CSS files, {js_count} JS files, and {img_count} images for {site_url}"
        )
...
def process_site(
    session: requests.Session, site_info: Dict, force_redownload: bool = False
) -> bool:
    ...
    if (
        ...
        and (project_dir / "info.html").exists()
        and (project_dir / "index.html").exists()
    ):
...
def gather_templates(
    ...
    if fresh_start:
        if PROJECT_TEMPLATES_DIR.exists():
            ...
            logger.warning(
                f"FRESH START: Removing all existing templates in {PROJECT_TEMPLATES_DIR}"
            )
            try:
    ...
    if processed_sites and not fresh_start:
        logger.info(
            f"Found {len(processed_sites)} already processed sites. Will skip them."
        )
    ...
    while True:
        ...
        if not html or not html.strip():
            empty_responses += 1
            logger.warning(
                f"Empty response (count: {empty_responses}/{max_empty_responses})"
            )
        ...
        if not sites:
            logger.warning(
                "No sites found in response, but response was not empty. Stopping."
            )
            break
        ...
        for site_info in sites:
            ...
            if skip_existing and site_info["site_url"] in processed_sites:
                total_skipped += 1
                logger.debug(
                    f"Skipping already processed site: {site_info['site_url']}"
                )
                continue
        ...
        if workers > 1 and sites_to_process:
            logger.info(
                f"Processing {len(sites_to_process)} sites with {workers} parallel workers..."
            )
            ...
            def process_site_wrapper(site_info):
                ...
                try:
                    success = process_site(
                        site_session, site_info, force_redownload=not skip_existing
                    )
                    return success, site_info
                except Exception as e:
                    logger.error(
                        f"Error processing {site_info.get('site_url', 'unknown')}: {e}"
                    )
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                # Submit all tasks
                future_to_site = {
                    executor.submit(process_site_wrapper, site_info): site_info
            ...
            for site_info in sites_to_process:
                success = process_site(
                    session, site_info, force_redownload=not skip_existing
                )
                if success:
                    total_processed += 1
                    if skip_existing:
    ...
    logger.info(
        f"Template gathering complete. Processed {total_processed} new sites, skipped {total_skipped} existing sites."
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Gather website templates from muuuuu.org"
    )
    parser.add_argument(
        "--start-from", type=int, default=0, help="Starting post number (default: 0)"
    )
    parser.add_argument(
        "--max-sites",
        type=int,
    ...
    if args.workers < 1:
        ...
        logger.warning(
            f"High workers value {args.workers} may cause rate limiting. Consider using 2-4."
        )
